...
export async function saveRecord(module,form){
 ...
 const data=Object.fromEntries(fieldsFor(module).map(f=>[f.key,String(form[f.key]??'').trim()]));data.attachments=form.attachments||[];data.imported_fields=form.imported_fields&&typeof form.imported_fields==='object'?form.imported_fields:{};if(form.source_sheet)data.source_sheet=form.source_sheet;if(form.source_row)data.source_row=form.source_row;
